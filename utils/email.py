...
Uses Resend (https://resend.com) — verified domain: densparkstudio.com

Required env vars in cPanel → Setup Python App → Environment Variables:
    RESEND_API_KEY   — from resend.com dashboard
    ADMIN_EMAIL      — where notifications are delivered
    RESEND_FROM      — verified sender email

NOTE: Threading is disabled for HostPinnacle shared hosting.
Passenger (cPanel's app runner) kills daemon threads before they complete,
so we send synchronously instead. Adds ~300ms to the request but reliable.
"""
...
def _get_config():
    return {
        ...
        'admin_email': os.environ.get('ADMIN_EMAIL', 'densparkstudio@gmail.com'),
        'from_email':  os.environ.get('RESEND_FROM',  'notifications@densparkstudio.com'),
    }


# =============================================================================
#  CORE SEND — synchronous (required for Passenger/shared hosting)
# =============================================================================

def _send(subject: str, plain: str, html: str) -> bool:
    """Send email via Resend API. Synchronous — safe for shared hosting."""
    cfg = _get_config()
    ...
    try:
        ...
        current_app.logger.info(f'Email sent: {subject} → {cfg["admin_email"]}')
        return True
...
#  Triggered when a client submits the booking form
# =============================================================================
...
<body>
  <div class="container">
    ...
    <div class="footer">Denspark Studio Admin · Automated notification · densparkstudio.com</div>
  </div>
</body>
</html>"""

        return _send(subject, plain, html)
